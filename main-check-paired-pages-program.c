...
uint8_t pageprogramtimes[Npages] = {11,11,
                                    11,11,30,60, // 6
                                    11,11,30,60,
                                    11,11,30,60,
                                    11,11,30,60,
    	...
    	if (usb_read_ready()) {
            ...
            if (strncmp((char *) cmd, "cppp", 4) == 0) {
                ...
                for (j = 0; j < blocklistsize; j++) {
                    ...
                    // program pages 0-1. 
                    // partial program page 2
                    // paired pages are 2 3 8 9
                    // read out pages 2 3 8 9

                    memset(write_buffer, 0x55, Nbyte); // write this
                    ...
                    //partial program page 9
                    memset(write_buffer, 0x00, Nbyte);
                    address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) 9) << PAGEOFFSET);
                    partial_write(address, Nbyte, write_buffer, 20000); // page 2 should be fast
                
                    // address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) 3) << PAGEOFFSET);
                    // partial_write(address, Nbyte, write_buffer, 6000); // page 2 should be fast


                    //read out pages 9 2 3 8
                    ...
                    address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) 2) << PAGEOFFSET);
                    read(address, Nbyte, read_buffer);
                    read(address, Nbyte, read_buffer);
                    usb_write(read_buffer, Nbyte);
                    //usb_write(readbufNbytebaseptr, Nbytespare);
                    ...
                    address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) 8) << PAGEOFFSET);
                    read(address, Nbyte, read_buffer);
                    usb_write(read_buffer, Nbyte);
                    //usb_write(readbufNbytebaseptr, Nbytespare);
                    usb_write((uint8_t *) "Done.", 5);
...
/*            else if (strncmp((char *) cmd, "cpp1", 4) == 0) {
                ...
                for (j = 0; j < blocklistsize; j++) {
                    ...
                    // address of block
                    address = address0 | (((uint32_t) blocklist[j] ) << BLOCKOFFSET ) | (((uint32_t) 0) << PAGEOFFSET );
                    ...
                    // program pages 0-8. 
                    // partial program page 9
                    // paired pages are 2 3 8
                    // read out pages 2 3 8

                    memset(write_buffer, 0x55, Nbyte); // write this

                    for (i = 0; i < 9; i++) {

                        address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) i) << PAGEOFFSET);
                        complete_write(address, Nbyte, write_buffer, otimep);
                    ...
                    memset(write_buffer, 0x00, Nbyte);
                    address = address0 | (((uint32_t) block) << BLOCKOFFSET) | (((uint32_t) 9) << PAGEOFFSET);
                    partial_write(address, Nbyte, write_buffer, 18000);
