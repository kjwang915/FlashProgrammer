...
#define pecyclestotry 40000000 // 40 million
...
#define pptimessize 13
// 13 partial program times
uint16_t pptimes[pptimessize] = {2000, 2250, 2500, 2750, 3000, 3250, 3500, 3750, 4000, 4500, 5000, 5500, 6000};
// 10blocks, 26 pages each - 160 pages total
uint16_t blocklist[blocklistsize] = {122, 531, 1647, 1686, 2601, 2610, 3156, 3192, 3275, 4049}; 
...
uint16_t entireblocklist[entireblocklistsize] = {500,  2750}; // 500-512, 2750-2762
...
uint8_t * readbuf2048ptr = (read_buffer + 2048);

// address from which things are derived. I'm pretty sure this is all 0
// I'm not sure why we have this
uint32_t address0 = (((uint32_t) 0x00) << 26) | (((uint32_t) 0x00) << 18) | (((uint32_t) (0x00)) << 12); 
...
int main(void) {
    ...
    while (1) {
    	if (usb_read_ready()) {
            ...
            //         result = partial_write(address, 2112, write_buffer, pptime);
            ...
            //             usb_write(read_buffer, 2048);
            //             usb_write(readbuf2048ptr, 64);                        
...
/*            else if (strncmp((char *) cmd, "pe", 2) == 0) {
                ...
                for (i = 0; i < eetimessize; i++) {
                    ...
                    address = address0 | (((uint32_t) blocklist[i]) << 18 );
                    ...
                    if (perase) {
                        ...
                        for (j = 0; j < Npages; j++) {
                            address = address0 | (((uint32_t) blocklist[i]) << 18 ) | (((uint32_t) j) << 12);
                            result = complete_write(address, 2112, write_buffer, otimep);
                        }
                    ...
                    for (k = 0; k < pptimessize; k++) {
                        ...
                        address = address0 | (((uint32_t) blocklist[i]) << 18 ) | (((uint32_t) pagelist[k]) << 12);
                    ...
                    for (k = 0; k < pptimessize; k++) {
                        ...
                        address = address0 | (((uint32_t) blocklist[i]) << 18 ) | (((uint32_t) secondpagelist[k]) << 12);
            ...
            // ************************************************************************
            /*
            else if (strncmp((char *) cmd, "m", 1) == 0) { 
            		...
            		for (j = 0; j < blocklistsize; j++) {
            			...
            			if (pptimes[i] != 6000) {
            				...
            				for (k = 0; k < Npages; k++) {
            					...
            					address = address0 | (((uint32_t) blocklist[j] ) << 18 ) | (((uint32_t) k) << 12 );
            					//timer = clock();
            			...
            			address = address0 | (((uint32_t) blocklist[j] ) << 18 ) | (((uint32_t) (0x00)) << 12 );
   ...
   //              usb_write(readbuf2048ptr, 64);
            ...
            /*
                ...
                while( 1 ) {
                    ...
                    if (blockyes && pageyes) {
                        usb_write((uint8_t *) "BPRD", 4); // block page read
                ...
                // write 0x00 ?
                    ...
                    memset(write_buffer, 0x00, 2112);
                    complete_write(address, 2112, write_buffer, otimep);
                    usb_write(otimep, 4);
                }
                // erase?
                else if (op == 3) {
