	...
	while (1) {
		if (usb_read_ready()) {
			...
			if (strncmp((char *)cmd, "m", 1) == 0) {
				...
				read(address, 2112, read_buffer); // read
				...
				complete_erase(address, otime1);
				
				memset(write_buffer, 0x00, 2112);
				partial_write(address,2112,write_buffer,otime1,2500);
 ...
 */
uint8_t partial_write(uint32_t address, uint32_t count, const uint8_t *src, uint8_t *otime1, uint32_t tprogram) {
	uint32_t i, otime;
	uint8_t result;
