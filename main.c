	...
	while (1) {
		if (usb_read_ready()) {
			...
			if (strncmp((char *)cmd, "m", 1) == 0) {
				...
				for (i=0;i<1;i++)
				{
					for (block=blocks;block<blockf;block=block+1)
					{
				...
				//stress part
				for (block=blocks; block<blockf;block=block+1)   //2 blocks, each characterize 10 times, which is equal to 20 blocks in time
				{									
					//info hiding by stress	
					for (j=0; j<625; j++) //5,000 pe stress now, perhaps at such high stress, we should use a shorter program time
					{	
						address=address0 | (((uint32_t) block) << 18);
				...
				for (i=0;i<1;i++)
				{
					for (block=610;block<710;block=block+1)
					{
						//complete write all of the block, prevent over erase attack
						for (page=0;page<64;page=page+1)  //64 pages
						{						
