			...
			if (strncmp((char *)cmd, "m", 1) == 0) {
				...
				for (dd=0; dd<3; dd++)  //2^11=2048 random pe cycles, which also means 5*12=60 blocks, 65 blocks in total
				{
					...
					for (i=0;i<1;i++)
					{
						...
						{
							//initial erase, this erase and following program emulated one normal usage
				...
				for (dd=0; dd<8; dd++)  //2^11=2048 random pe cycles, which also means 5*12=60 blocks, 65 blocks in total
				{
					memset(write_buffer2, 0x00, mylen ); 
					//characterization part	
					for (i=0;i<1;i++)
